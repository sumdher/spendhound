...
class PartnerRequest(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(20), nullable=False, server_default=PARTNER_STATUS_PENDING
    )
    ...
    requester: Mapped[User] = relationship(
        "User", foreign_keys=[requester_id], back_populates="sent_partner_requests"
    )
    recipient: Mapped[User | None] = relationship(
        "User", foreign_keys=[recipient_id], back_populates="received_partner_requests"
    )

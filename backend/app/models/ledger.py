...
class Ledger(Base):
    ...
    creator: Mapped[User] = relationship(
        "User", foreign_keys=[created_by], back_populates="created_ledgers"
    )
    ...
    audit_logs: Mapped[list[LedgerAuditLog]] = relationship(
        "LedgerAuditLog", back_populates="ledger", cascade="all, delete-orphan"
    )
...
class LedgerMembership(Base):
    ...
    ledger_id: Mapped[uuid.UUID] = mapped_column(
        Uuid(as_uuid=True), ForeignKey("ledgers.id", ondelete="CASCADE"), nullable=False, index=True
    )

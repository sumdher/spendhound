...
class Settings(BaseSettings):
    ...
    ollama_url: str = Field(
        default="http://host.docker.internal:11434", description="Ollama base URL"
    )
    ...
    monthly_reports_enabled: bool = Field(
        default=False, description="Enable monthly report delivery job"
    )
    monthly_reports_timezone: str = Field(
        default="UTC", description="IANA timezone used to compute the reporting month"
    )
    monthly_reports_frontend_pdf_url: str = Field(
        default="", description="Internal frontend endpoint used to render monthly report PDFs"
    )
    monthly_reports_frontend_token: str = Field(
        default="", description="Shared secret token sent to the internal frontend PDF endpoint"
    )
    monthly_reports_frontend_token_header: str = Field(
        default="X-SpendHound-Internal-Token",
        description="Header name used for monthly report frontend authentication",
    )
    monthly_reports_frontend_timeout_seconds: int = Field(
        default=60, description="Timeout for internal frontend monthly report PDF requests"
    )
    recurring_generation_enabled: bool = Field(
        default=False, description="Enable automatic generation of recurring expenses"
    )
    recurring_generation_timezone: str = Field(
        default="UTC",
        description="IANA timezone used to compute recurring expense generation months",
    )
    ...
    ollama_embedding_model: str = Field(
        default="embeddinggemma:latest", description="Ollama model for item embeddings (RAG)"
    )
    embedding_dimensions: int = Field(
        default=768, description="Vector dimensions produced by the embedding model"
    )
    rag_similarity_threshold: float = Field(
        default=0.22,
        description="Cosine distance threshold for RAG match (lower = stricter; 0.0=identical, 2.0=opposite)",
    )
    ...
    rate_limit_chat_per_minute: int = Field(
        default=20, description="Chat stream requests per user per minute"
    )
    rate_limit_upload_per_minute: int = Field(
        default=3, description="Receipt uploads per user per minute"
    )
    rate_limit_auth_per_minute: int = Field(
        default=10, description="Auth requests per IP per minute"
    )

    # ── Database connection pool ──────────────────────────────────────────────
    db_pool_size: int = Field(default=20, description="SQLAlchemy async connection pool base size")
    db_max_overflow: int = Field(
        default=40, description="Max extra connections beyond db_pool_size"
    )

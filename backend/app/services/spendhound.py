...
def normalize_recurring_settings(
    cadence: str | None,
    *,
...
def signed_amount(value: Decimal | float, transaction_type: str) -> float:
    ...
    return round(
        amount
        if normalize_transaction_type(transaction_type) == TRANSACTION_TYPE_CREDIT
        else -amount,
        2,
    )
...
def _detect_recurring_cadence(group: list[Expense]) -> str | None:
    ...
    if any(abs(amount - avg_amount) / avg_amount > 0.05 for amount in amounts):
        return None
    gaps = [
        (group[index].expense_date - group[index - 1].expense_date).days
...
def fuzzy_text_match(value: str, pattern: str) -> bool:
    ...
    for token in pattern_tokens:
        if any(
            SequenceMatcher(None, token, candidate).ratio() >= 0.9 for candidate in value_tokens
        ):
            return True
    return False


async def expense_category_name(
    db: AsyncSession, expense: Expense, category_name: str | None = None
) -> str | None:
...
async def get_category_by_name(
    db: AsyncSession, user_id: uuid.UUID, name: str | None, *, transaction_type: str | None = None
) -> Category | None:
    if not name:
        return None
    statement = select(Category).where(
        Category.user_id == user_id, Category.name.ilike(name.strip())
    )
    if transaction_type:
        statement = statement.where(
            Category.transaction_type == normalize_transaction_type(transaction_type)
        )
...
def matches_item_keyword_rule(description: str, rule: ItemKeywordRule) -> bool:
    ...
    if rule.pattern_type == "abbrev":
        ...
        if len(kw) < 2:
            return False
        return any(
            _is_subsequence(kw, token) for token in normalize_match_text(description).split()
        )
            ...
            if rule.is_global:
                ...
                category_result = await db.execute(
                    select(Category).where(
                        Category.id == rule.category_id, Category.user_id == user_id
                    )
                )
            category = category_result.scalar_one_or_none()
            if category is not None and (
                normalized_type is None or category.transaction_type == normalized_type
            ):
                return category
    return None


async def find_matching_item_subcategory(
    db: AsyncSession, user_id: uuid.UUID, description: str | None
...
async def upsert_item_keyword_rule_from_correction(
    ...
    *,
    ...
    """
    tokens = [
        t for t in normalize_match_text(description).split() if len(t) >= 4 and not t.isdigit()
    ]
    if not tokens:
        return None
    ...
    *,
    ...
    if category_name:
        category = await get_or_create_category(
            db, user_id, category_name, transaction_type=normalized_type
        )
...
def serialize_budget(
    budget: Budget, category_name: str | None = None, actual: float | None = None
) -> dict:
...
def serialize_expense(
    ...
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "recurring_source_expense_id": str(expense.recurring_source_expense_id)
        if expense.recurring_source_expense_id
        else None,
        "auto_generated": expense.auto_generated,
        "generated_for_month": expense.generated_for_month.isoformat()
        if expense.generated_for_month
        else None,
        "cadence_interval": expense.cadence_interval,
        "prepaid_months": expense.prepaid_months,
        "prepaid_start_date": expense.prepaid_start_date.isoformat()
        if expense.prepaid_start_date
        else None,
...
async def replace_expense_items(
    db: AsyncSession, expense: Expense, items: list[Any] | None, *, category_name: str | None = None
) -> None:
    ...
    for item in items or []:
        ...
        if is_grocery_expense and not subcategory:
            matched_subcategory = await find_matching_item_subcategory(
                db, expense.user_id, description
            )
...
async def recompute_recurring_expenses(db: AsyncSession, user_id: uuid.UUID) -> None:
    result = await db.execute(
        select(Expense).where(Expense.user_id == user_id).order_by(Expense.expense_date.asc())
    )
    ...
    for expense in expenses:
        ...
        if expense.cadence_override:
            ...
            if cadence_is_recurring(overridden_cadence):
                ...
                expense.recurring_group = (
                    f"manual:{merchant_key}:{currency}:{transaction_type}:{overridden_cadence}"
                )
            continue
    ...
    for (merchant_key, currency, transaction_type), group in grouped.items():
        ...
        recurring_group = (
            f"{merchant_key}:{currency}:{transaction_type}:{detected_cadence}:{avg_amount:.2f}"
        )
    ...
    for expense in expenses:
        if (
            normalize_transaction_type(expense.transaction_type) != TRANSACTION_TYPE_DEBIT
            or expense.cadence != CADENCE_ONE_TIME
        ):
...
def recurring_expense_is_due_for_month(expense: Expense, target_month: date) -> bool:
    ...
    if cadence == CADENCE_CUSTOM:
        ...
        months_since = (target_month.year - template_month.year) * 12 + (
            target_month.month - template_month.month
        )
        return months_since % interval == 0
    return False


async def generate_recurring_expenses_for_month(
    db: AsyncSession, user_id: uuid.UUID, target_month: date
) -> list[Expense]:
...
def apply_expense_filters(
    ...
    *,
    ...
    if month and month not in {"all", "all_time"}:
        ...
        statement = statement.where(
            Expense.expense_date >= start, Expense.expense_date < next_month(start)
        )
    if category_id:
        statement = statement.where(Expense.category_id == category_id)
    if transaction_type:
        statement = statement.where(
            Expense.transaction_type == normalize_transaction_type(transaction_type)
        )
    if cadence and cadence not in {"all", "auto"}:
        statement = statement.where(Expense.cadence == normalize_cadence(cadence))
    if review_only:
        statement = statement.where(
            (Expense.needs_review.is_(True)) | (Expense.category_id.is_(None))
        )
    if search:
        like_value = f"%{search.strip()}%"
        statement = statement.where(
            Expense.merchant.ilike(like_value) | Expense.description.ilike(like_value)
        )
    return statement
...
async def delete_orphaned_category_rules(db: AsyncSession, category_id: uuid.UUID) -> None:
    await db.execute(
        delete(MerchantRule).where(
            MerchantRule.category_id == category_id, MerchantRule.is_active.is_(False)
        )
    )

...
def _item(
    desc: str, qty: float | None, unit: float | None, total: float, subcat: str | None = None
) -> dict:
    return {
        "description": desc,
        "quantity": qty,
        ...
        "total": total,
        "subcategory": subcat,
        "subcategory_confidence": 0.92 if subcat else None,
    }
...
_M0_GROCERY_ITEMS = [
    ...
    _item(
        "Fresh summer truffle 30g (Alfred's 'non-negotiable' splurge)",
        1,
        54.00,
        54.00,
        "Condiments & Spices",
...
_M2_PETER_GROCERY_ITEMS = [
    ...
    _item(
        "Gatorade Blue Bolt x6 (electrolyte replenishment — not labeled as such)",
        6,
        1.29,
        7.74,
        "Beverages",
...
def _build_expenses(
    ledger_manor: uuid.UUID, ledger_fox: uuid.UUID, ledger_jl: uuid.UUID
) -> list[dict]:
    return [
        ...
        _e(
            "m2-01",
            "Wayne Manor Property Management",
            "3200.00",
            "Housing",
            _day(2, 1),
        ...
        _e(
            ...
            description="Wayne Manor monthly provisions — Alfred's bulk order",
        ),
        _e(
            "l-manor-m2-02",
            "Gotham Green Grounds Ltd.",
            "850.00",
            "Other",
            _day(2, 15),
            ledger_id=ledger_manor,
            description="Manor grounds bi-monthly maintenance & topiary",
        ),
        # Fox-Wayne Labs ledger — month -2
        _e(
            "l-fox-m2-01",
            "Gotham Advanced Composites Inc.",
            "12400.00",
            "WayneTech R&D",
            _day(2, 22),
            ledger_id=ledger_fox,
            is_major=True,
        ...
        _e(
            ...
            _day(2, 29),
            ledger_id=ledger_jl,
            description="JL HQ monthly coffee & snack fund. Aquaman drinks an unreasonable amount.",
        ),
        # ── Month -1 (last month) ────────────────────────────────────────────────
        _e(
            "inc-m1-01",
        ...
        _e(
            ...
            _day(1, 6),
            ledger_id=ledger_manor,
            description="Wayne Manor monthly provisions",
        ),
        _e(
            "l-manor-m1-02",
            "CaveClean Industrial Services",
            "440.00",
            "Groceries",
            _day(1, 28),
            ledger_id=ledger_manor,
            description="Batcave deep-clean supplies — industrial-grade, bat-safe certified",
        ),
        # Fox-Wayne Labs ledger — month -1
        _e(
            "l-fox-m1-01",
            "CognitionTech Computing",
            "8900.00",
            "WayneTech R&D",
            _day(1, 16),
            ledger_id=ledger_fox,
        ...
        _e(
            "m0-11",
            "The Grand Ballroom, Gotham Plaza",
            "18500.00",
            "Entertainment",
            _day(0, 15),
            is_major=True,
        ...
        _e(
            "m0-13",
            "Gotham Addiction Recovery Center",
            "100000.00",
            "Wayne Foundation",
            _day(0, 17),
            is_major=True,
...
def _build_peter_expenses(
    ledger_lab: uuid.UUID, ledger_avengers: uuid.UUID, ledger_may: uuid.UUID
) -> list[dict]:
    return [
        # ── Month -2 ─────────────────────────────────────────────────────────────
        _e(
...
async def seed_demo_data(db: AsyncSession) -> User:
    ...
    await _get_or_create_user(
        db,
        ALFRED_ID,
        "alfred.pennyworth@waynemanor.com",
        "Alfred Pennyworth",
    ...
    await _get_or_create_user(
        db,
        LUCIUS_ID,
        "lucius.fox@wayneenterprises.com",
        "Lucius Fox",
    ...
    await _ensure_partner_requests(
        db,
        bruce.id,
        [
    ...
    count_result = await db.execute(
        select(func.count(Expense.id)).where(Expense.user_id == bruce.id)
    )
...
async def reset_demo_data(
    ...
    cat_by_name: dict[str, uuid.UUID] | None = None,
) -> None:
    ...
    await _seed_expenses_and_budgets(
        db, uid, cat_by_name, ledger_manor.id, ledger_fox.id, ledger_jl.id
    )
    await db.commit()
...
async def seed_peter_data(db: AsyncSession) -> User:
    ...
    await _get_or_create_user(
        db,
        HAPPY_ID,
        "happy.hogan@starkindustries.com",
        "Happy Hogan",
    ...
    needs_seed = False
    count_result = await db.execute(
        select(func.count(Expense.id)).where(Expense.user_id == peter.id)
    )
    if (count_result.scalar() or 0) == 0:
        needs_seed = True
    else:
...
async def reset_peter_data(
    ...
    ledger_avengers = await _create_ledger(
        db, _LEDGER_AVENGERS_ID, "Avengers Auxiliary", uid, [HAPPY_ID]
    )
    ledger_may = await _create_ledger(
        db, _LEDGER_MAY_ID, "Aunt May's Household", uid, [AUNT_MAY_ID]
    )
